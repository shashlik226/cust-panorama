...
var TournamentJournal = ( function()
{
    ...
    var m_tournamentId = 0;

    var _Init = function()
    {
        var journalId = $.GetContextPanel().GetAttributeString( "journalid", '' );
        m_tournamentId = InventoryAPI.GetItemAttributeValue( journalId, "tournament event id" ) > 17 ? InventoryAPI.GetItemAttributeValue( journalId, "tournament event id" ) : $.GetContextPanel().GetAttributeString( "tournamentid", g_ActiveTournamentInfo.eventid );
		...
		m_tokenItemDefName = 'tournament_pass_' + g_Tournaments[m_tournamentId].g_ActiveTournamentInfo.location + '_charge';
        ...
        var oWinningTeam = !_IsValidJournalId( journalId ) ? null : EventUtil.GetTournamentWinner( m_tournamentId, 1 );
        ...
        _SetFaqBtn(m_tournamentId, bNoItemsOnSale);
        _UpdateStoreItems( bNoItemsOnSale );
        _UpdateActivateBtn();
        _SetBackgroundMovie( m_tournamentId );
        ...
        _SetThesholdText( journalId, m_tournamentId );
        _UpdateChallengesList( journalId, m_tournamentId );
        _SouvenirsEarned( journalId, m_tournamentId );
        _SouvenirsImage( m_tournamentId );
        _SetEventSticker( m_tournamentId );
        _UpdateSetChargesBtn( ( m_activeTournament === m_tournamentId ) ? InventoryAPI.GetFauxItemIDFromDefAndPaintIndex( g_Tournaments[m_tournamentId].g_ActiveTournamentInfo.itemid_charge, 0 ) : null );
        _UpdateApplyCharges();
        _SetWinners( oWinningTeam, m_tournamentId, journalId );
    ...
    {
        var tournamentId = CustomAPI.GetActiveTournamentEventID();

        return ( ( tournamentId !== 0 ) &&
            ( '' !== StoreAPI.GetStoreItemSalePrice(
                InventoryAPI.GetFauxItemIDFromDefAndPaintIndex(
                    g_Tournaments[m_tournamentId].g_ActiveTournamentStoreLayout[ 0 ][ 0 ], 0 ), 1, '' ) ) );
    };

    var GetPassToActivate = function()
    {
        var journalId = $.GetContextPanel().GetAttributeString( "journalid", '' );
        var id = InventoryAPI.GetActiveTournamentCoinItemId( g_Tournaments[m_tournamentId].g_ActiveTournamentInfo.eventid * -1 );
    ...
    var _SetBackgroundMovie = function( id )
    {
        if ( id > 17 )
        {
            $.GetContextPanel().FindChildInLayoutFile( 'id-tournament-journal-container' ).style.backgroundImage = "url( 'file://{images}/tournaments/events/bg_" + id + "_fullscreen.png' );";
        }
        else if ( id == 16 )
        {
        ...
        {
            $.GetContextPanel().FindChildInLayoutFile( 'id-tournament-journal-container' ).style.backgroundImage = "url( 'file://{resources}/videos/tournament_bg.webm' );";
        }
    };
    ...
    var _SetTitle = function( journalId )
    {
        $.GetContextPanel().FindChildInLayoutFile( 'id-tournament-journal-title' ).text = !_IsValidJournalId( journalId ) ? 
            $.Localize('CSGO_TournamentPass_' + g_Tournaments[m_tournamentId].g_ActiveTournamentInfo.location + '_store_title') : 
            ItemInfo.GetName( journalId );
    };

    var _SetSubtitle = function( journalId )
    {
        var srtText = !_IsValidJournalId( journalId ) ? '#CSGO_TournamentPass_' + g_Tournaments[m_tournamentId].g_ActiveTournamentInfo.location + '_store_desc' : '#tournament_coin_desc_token';
        $.GetContextPanel().FindChildInLayoutFile( 'id-tournament-journal-subtitle' ).text = $.Localize( srtText );
    };

    var _SetModel = function( id )
    {
        var fakeId = InventoryAPI.GetFauxItemIDFromDefAndPaintIndex( g_Tournaments[m_tournamentId].g_ActiveTournamentInfo.itemid_coins[ g_Tournaments[m_tournamentId].g_ActiveTournamentInfo.itemid_coins.length - 1 ], 0 );                                                                       
    ...
    var _SouvenirsImage = function( tournamentId )
    {
        ...
        {
            elImage.itemid = InventoryAPI.GetFauxItemIDFromDefAndPaintIndex( g_Tournaments[m_tournamentId].g_ActiveTournamentInfo.itemid_charge, 0 );
        }
    };
    ...
    var _SetFaqBtn = function( tournamentId, bNoItemsOnSale )
    {
        ...
        btn.SetPanelEvent( 'onactivate', function(){
            SteamOverlayAPI.OpenURL( 'https://store.steampowered.com/sale/csgo' + g_Tournaments[m_tournamentId].g_ActiveTournamentInfo.location );
        });
    ...
    var _UpdateStoreItems = function( bNoItemsOnSale )
    {
		...
		var fnCreateOffering = function ( ids, i ) {
            if ( !bCanSellCapsules && ( i >= g_Tournaments[m_tournamentId].g_ActiveTournamentInfo.num_global_offerings ) )
            {                                                                                                      
                return;
            }
            ...
            {
				var itemid = InventoryAPI.GetFauxItemIDFromDefAndPaintIndex( g_Tournaments[m_tournamentId].g_ActiveTournamentStoreLayout[ i ][ 0 ], 0 );
				var bContainsJustChampions = ( typeof g_Tournaments[m_tournamentId].g_ActiveTournamentStoreLayout[ i ][ 1 ]  === 'string' );
				var linkedid = bContainsJustChampions ? itemid
					: InventoryAPI.GetFauxItemIDFromDefAndPaintIndex( g_Tournaments[m_tournamentId].g_ActiveTournamentStoreLayout[ i ][ 1 ], 0 );
                ...
                elItem.Data().oData = {
					...
					linkpricing: 'reverse',
					usetinynames: true,
                    usegroupname: g_Tournaments[m_tournamentId].g_ActiveTournamentStoreLayout[ i ][ 2 ],
				...
				var defidxChampionHighlight = [ g_Tournaments[m_tournamentId].g_ActiveTournamentStoreLayout[ g_Tournaments[m_tournamentId].g_ActiveTournamentStoreLayout.length - 1 ][0] ];
                if ( g_Tournaments[m_tournamentId].g_ActiveTournamentStoreLayout[ i ][ 0 ] === defidxChampionHighlight )
                {
        ...
        g_Tournaments[m_tournamentId].g_ActiveTournamentStoreLayout.forEach( fnCreateOffering );
    };
