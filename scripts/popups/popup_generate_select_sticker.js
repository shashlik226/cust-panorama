...
var PopupGenerateSelectSticker = ( function()
{
    ...
    function _CreatePanels() {
		...
		var tournamentfilter = $.GetContextPanel().FindChildInLayoutFile('hideTournamentStickers').IsSelected();

        var filteredItems = items.filter(item => {
            var matchesName = !itemNamefilter || (item.itemName && item.itemName.toLowerCase().includes(itemNamefilter));
            var matchesTournament = !tournamentfilter || !item.isTournamentSticker;

            return matchesName && matchesTournament;
        });
        ...
        filteredItems.forEach(item => {
            ...
            {
    	        UiToolkitAPI.ShowSimpleContextMenu( '', 'StickerContextMenu', [
		        	{ label: 'Create sticker', jsCallback: function() {
                        GameInterfaceAPI.ConsoleCommand(`rcon give_item 1209 1 sticker0=${item.id}`);
                    } },
		        	...
		        	{ label: 'Copy create command to Clipboard', jsCallback: function() {
		        		SteamOverlayAPI.CopyTextToClipboard( `give_item 1209 1 sticker0=${item.id}` );
		        	} }
		        ]);
            }.bind( undefined ) );
	...
	function _Init()
	{
        ...
        if(globalObject.hasOwnProperty('selectStickerCache')) {
            items = globalObject['selectStickerCache'];
        } else {
	    	for (let i = 0; i < 7232; i++) {
                ...
                var isTournamentSticker = JSON.stringify(oTags).includes('TournamentTeam');

	    		items.push({name: itemName, id: i, itemid: fauxItemId, isTournamentSticker: isTournamentSticker});
	    	}
            globalObject['selectStickerCache'] = items;
        }
        _CreatePanels();
	}
