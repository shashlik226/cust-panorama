...
var ItemContextEntires = ( function (){
	...
	var _Entries = [
		...
		},
		{
			...
			AvailableForItem: function ( id ) {
				return ( ItemInfo.ItemDefinitionNameSubstrMatch(id, 'tournament_journal_') &&
					InventoryAPI.GetItemAttributeValue( id, "tournament event id" ) > 17 );
			},
			OnSelected:  function ( id ) {
				UiToolkitAPI.ShowCustomLayoutPopupParameters(
					'',
		...
		},
		{
			...
			AvailableForItem: function ( id ) {
				return GameInterfaceAPI.GetSettingString( "cl_inventory_debug_tooltip") == "1";
			},
			OnSelected: function ( id ) {
				$.DispatchEvent( 'ContextMenuEvent', '' );
				SteamOverlayAPI.CopyTextToClipboard( id )
			}
		},
		{
			name: 'Copy Item Schema JSON',
			hascustomname: true,
			AvailableForItem: function ( id ) {
				return GameInterfaceAPI.GetSettingString( "cl_inventory_debug_tooltip") == "1";
			},
			OnSelected: function ( id ) {
				$.DispatchEvent( 'ContextMenuEvent', '' );
				SteamOverlayAPI.CopyTextToClipboard( InventoryAPI.BuildItemSchemaDefJSON( id ) );
			}
		}
