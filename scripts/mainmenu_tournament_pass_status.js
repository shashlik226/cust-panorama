...
var MainMenuTournamentPassStatus = ( function()
{
	...
	var _m_tournamentId = 0;
	var _Init = function()
	{
		_m_tournamentId = _m_cp.Data().tournamentId;
	...
	var _UpdatePanel = function()
	{
		...
		var id = InventoryAPI.GetActiveTournamentCoinItemId( _m_tournamentId );
		...
		$.GetContextPanel().style.backgroundImage = 'url("file://{images}/store/tournament_items_' + _m_tournamentId + '.png")';
	...
	var _AddItemImages = function()
	{
		...
		aImages[ 0 ].itemid = InventoryAPI.GetFauxItemIDFromDefAndPaintIndex( g_Tournaments[_m_tournamentId].g_ActiveTournamentStoreLayout[ 0 ][ 1 ], 0 );
		aImages[ 1 ].itemid = InventoryAPI.GetFauxItemIDFromDefAndPaintIndex( g_Tournaments[_m_tournamentId].g_ActiveTournamentStoreLayout[ 1 ][ 0 ], 0 );
		aImages[ 2 ].itemid = InventoryAPI.GetFauxItemIDFromDefAndPaintIndex( g_Tournaments[_m_tournamentId].g_ActiveTournamentStoreLayout[ 2 ][ 0 ], 0 );
		if(_m_tournamentId < 18) {
			aImages[ 3 ].itemid = InventoryAPI.GetFauxItemIDFromDefAndPaintIndex( g_Tournaments[_m_tournamentId].g_ActiveTournamentStoreLayout[ 3 ][ 0 ], 0 );
		}
	};
	...
	var _ItemsOnSale = function()
	{
		...
		elPrecent.SetHasClass( 'hidden', true );
			...
			var reduction = ItemInfo.GetStoreSalePercentReduction( g_Tournaments[_m_tournamentId].g_ActiveTournamentStoreLayout[1][0], 1 );
	...
	var _ShowTournamentJournalPanel = function ( id )
	{
		...
		UiToolkitAPI.ShowCustomLayoutPopupParameters(
			...
			`journalid=${id}&tournamentid=${_m_tournamentId}`
		);
	};
